...
foreach subid (*)
	...
	set outDir=/danl/SB/PaulCompileTGNG/mri_scripts/cpacPipelines/glm/output/${subid}/5_afni_6motion_regOutConfounds_highpass_1G_no_errors/
	...
	if (-f ${inputFunc}) then
		...
		if (! -d ${outDir}) then
			...
			3dDeconvolve -input ${inputFunc} \
					...
					-gltsym 'SYM: Fear Neutral' -glt_label 4 AllFaces || ${subid} >> afniCrashLog.txt
		else 
