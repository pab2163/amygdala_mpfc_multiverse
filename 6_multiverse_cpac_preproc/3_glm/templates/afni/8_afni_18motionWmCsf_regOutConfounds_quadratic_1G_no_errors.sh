...
foreach subid (*)
	...
	set outDir=/danl/SB/PaulCompileTGNG/mri_scripts/cpacPipelines/glm/output/${subid}/8_afni_18motionWmCsf_regOutConfounds_quadratic_1G_no_errors/
	...
	if (-f ${inputFunc}) then
		...
		if (! -d ${outDir}) then
			...
			3dDeconvolve -input ${inputFunc} \
					...
					-gltsym 'SYM: Fear Neutral' -glt_label 4 AllFaces || ${subid} >> afniCrashLogPipeline8.txt
		else 
		...
		echo ${subid}
		echo ${subid} >> noBrainDataPipeline8.txt
	endif 
