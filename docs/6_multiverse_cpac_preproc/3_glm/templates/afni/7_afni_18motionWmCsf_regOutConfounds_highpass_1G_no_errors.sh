...
foreach subid (*)
	...
	set inputFunc=/danl/SB/PaulCompileTGNG/mri_scripts/cpacPreproc/cpacTesting2/output/pipeline_analysis_testing/${subid}_${subid}/functional_to_standard/_scan_rest/${subid}_tgng_fear_bold_calc_tshift_resample_volreg_calc_maths_antswarp_highpass.nii.gz
	...
	set nuisanceFile=/danl/SB/PaulCompileTGNG/mri_scripts/cpacPipelines/motionConfounds/nuisanceGLMFilesFear/${subid}_0.9_wmCsf18nuisancePlusConfounds.txt

	# Path to output directory
	set outDir=/danl/SB/PaulCompileTGNG/mri_scripts/cpacPipelines/glm/output/${subid}/7_afni_18motionWmCsf_regOutConfounds_highpass_1G_no_errors/
	...
	if (-f ${inputFunc}) then
		...
		if (! -d ${outDir}) then
			...
			3dDeconvolve -input ${inputFunc} \
					...
					-gltsym 'SYM: Fear Neutral' -glt_label 4 AllFaces || ${subid} >> afniCrashLogPipeline7.txt
		else 
		...
		echo ${subid}
		echo ${subid} >> noBrainDataPipeline7.txt
	endif 
