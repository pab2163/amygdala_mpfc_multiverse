...
for index, row in outDf.iterrows():
    ...
    for columnName in list(outDf.columns[1:]):
        try:
            ...
            outDf.loc[index, columnName] = getRoiStatFSL(subject = row['name'], pipeline = str(columnName).split('__')[0] + '.feat', roi = image.load_img(roiPath), contrastNum = 1, imageType = 'cope')
        except:
            print('ERROR!!')
outDf.to_csv('allHO_Amyg_FSL_Copes.csv', index = False)
...
for index, row in outDf.iterrows():
    ...
    for columnName in list(outDf.columns[1:]):
        try:
            roiPath = '/danl/SB/Investigators/PaulCompileTGNG/mri_scripts/4_roi_analysis/multiverseAmygROIs/amygMultiverseROIs_HO/' + str(columnName).split('__', 1)[1] + '.nii.gz'            
            outDf.loc[index, columnName] = getRoiStatFSL(subject = row['name'], pipeline = str(columnName).split('__')[0] + '.feat', roi = image.load_img(roiPath), contrastNum = 1, imageType = 'tstat')
        except:
            print('ERROR!!')
outDf.to_csv('allHO_Amyg_FSL_Tstats.csv', index = False)
...
for index, row in outDf.iterrows():
    ...
    for columnName in list(outDf.columns[1:]):
        try:
            roiPath = '/danl/SB/Investigators/PaulCompileTGNG/mri_scripts/4_roi_analysis/multiverseAmygROIs/amygMultiverseROIs_HO/' + str(columnName).split('__', 1)[1] + '.nii.gz'            
            outDf.loc[index, columnName] = getRoiStatAFNI(subject = row['name'], pipeline = str(columnName).split('__')[0], roi = image.load_img(roiPath), contrastNum = 1, imageType = 'fearCoef')
        except:
            print('ERROR!!')
outDf.to_csv('allHO_Amyg_AFNI_Coefs.csv', index = False)
...
for index, row in outDf.iterrows():
    ...
    for columnName in list(outDf.columns[1:]):
        try:
            roiPath = '/danl/SB/Investigators/PaulCompileTGNG/mri_scripts/4_roi_analysis/multiverseAmygROIs/amygMultiverseROIs_HO/' + str(columnName).split('__', 1)[1] + '.nii.gz'            
            outDf.loc[index, columnName] = getRoiStatAFNI(subject = row['name'], pipeline = str(columnName).split('__')[0], roi = image.load_img(roiPath), contrastNum = 1, imageType = 'fearTstat')
        except:
            print('ERROR!!')
outDf.to_csv('allHO_Amyg_AFNI_Tstats.csv', index = False)